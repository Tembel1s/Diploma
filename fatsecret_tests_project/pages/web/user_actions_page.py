...
class FoodSearch:
    ...
    def search_product(self, product):
        browser.element("#multiSearchText").should(be.clickable).click().send_keys(
            product
        ).press_enter()
    ...
    def check_search_results(self, product):
        ...
        browser.element(".generic.searchResult").all("tr").should(
            have.text(product).each
        )
        browser.element(".searchRelated").should(be.present).should(
        ...
        browser.element(f"[href='/calories-nutrition/search?q={product}&pg=1']").should(
            be.clickable
        ).click()

        browser.element(".generic.searchResult").all("tr").element_by(
            have.text(product)
        ).should(be.present)
        browser.element(".searchRelated").should(be.present).should(
            have.text("Related searches for: ")
...
class AddFood:
    ...
    def go_to_food_diary(self):
        ...
        browser.execute_script(
            "arguments[0].click();", browser.element('[href="/Diary.aspx?pa=fj"]')()
        )
    ...
    def choose_product(self, product):
        browser.element("#searchBfastExp").should(be.clickable).click().send_keys(
            product.name
        )
    ...
    def check_added_products_in_diary(self, *products):
        for product in products:
            browser.all(".generic.foodsNutritionTbl").element_by(
                have.text(product)
            ).should(be.visible)
...
class CaloriesCount:
    ...
    def get_product_calories_count(self, product):
        product_info = (
            browser.all(f'#rec_info_link_1_0_{product.id} [title="View info"]')
            .element_by(have.text("kcal"))
            .should(be.present)
        ...
        calories_quantity_number = "".join(
            filter(str.isdigit, calories_quantity_with_text_kcal)
        )
    ...
    def check_correct_calories_quantity_in_diary(self, quantity):
        browser.all(".foodsNutritionTbl").element_by(have.text("Cals")).element(
            ".."
        ).should(have.text(f"{quantity}"))

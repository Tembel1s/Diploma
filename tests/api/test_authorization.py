...
def test_authorization(url, api_request_html):
    data = {
    ...
    response_before_redirect = api_request_html(url, endpoint="/Auth.aspx?pa=s", method="POST", data=data,
                                                params=params)
    ...
    response_after_redirect = api_request_html(url, endpoint="/Auth.aspx?pa=s", method="POST", data=data)
...
def test_authorization_wrong_password(url, api_request_html):
    data = {
        "__EVENTTARGET": "ctl00$ctl11$Logincontrol1$Login",
        "ctl00$ctl11$Logincontrol1$Name": f"{valid_login}",
    ...
    response = api_request_html(url, endpoint="/Auth.aspx?pa=s", method="POST", data=data)
